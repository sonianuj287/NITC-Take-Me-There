...
const db = SQLite.openDatabase("finzz.db");
 ...
 display2() {
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM SaveLocations', [], (tx, results) => {
        var temp = [];
        for (let i = 0; i < results.rows.length; ++i) {
          temp.push({key:results.rows.item(i).id,label:results.rows.item(i).Locations,leftLabel: 'Go to Map', rightLabel: 'Delete'});
        }
        this.setState({
          data2: temp,
        });
    ...
    <View style = {styles.container}>
      <Card style={styles.inputContainer}>
        ...
        <View style={{flexDirection:"row"}}>
          <View style={{width:"80%"}}>
         <Button color="green" style={styles.button} title="Display" onPress={ this.display.bind(this), this.display2.bind(this) }/>
         </View>
         <Text>      </Text>
         <View style={{width:"15%"}}>
         ...
         <SwipeableFlatList
         style={{width:340},{backgroundColor:'black'}}
            data={this.state.data2}
            renderItem={({ item }) => (
                <Text style={{ height: 48 }}>{item.label}</Text>
            )}
            renderLeft={({ item }) => (
                <Text style={{ width: 220 },{color:'white'},{backgroundColor:'green'}}>{item.leftLabel}</Text>
            )}
            renderRight={({ item }) => (
                <Text style={{ width: 180 },{color:'white'},{backgroundColor:'red'}}>{item.rightLabel}</Text>
            )}
            backgroundColor={'Red'}
        />
